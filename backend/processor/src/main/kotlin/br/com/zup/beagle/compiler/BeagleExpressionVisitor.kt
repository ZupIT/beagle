...
internal class BeagleExpressionVisitor(processingEnvironment: ProcessingEnvironment, generatedKey: String) {
    private val elementUtils = processingEnvironment.elementUtils
    private val typeUtils = processingEnvironment.typeUtils
    private val generationDirectory = File(processingEnvironment.options[generatedKey]!!)
    private val visited = mutableSetOf<TypeMirror>()

    fun visit(element: TypeElement, suffix: String = ROOT_SUFFIX) {
    ...
    private fun createMainGenerator(element: TypeElement, suffix: String) =
        FileSpec.get(this.elementUtils.getPackageName(element), this.createType(element, suffix))
            .writeTo(this.generationDirectory)
    ...
    private fun createGenerationProperty(element: ExecutableElement, isRoot: Boolean) =
        PropertySpec.builder(
            element.fieldName.replace(INTERNAL, ""),
            this.createGenerationType(element.returnType)
        ).let {
