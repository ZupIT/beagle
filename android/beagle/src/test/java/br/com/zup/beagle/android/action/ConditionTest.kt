...
class ConditionTest : BaseTest() {
    ...
    fun `execute should call onTrue attribute`() {
        ...
        every { condition.handleEvent(rootView, view, listOf(onTrue), analyticsHandleEvent = any()) } just Runs

        // When
        condition.execute(rootView, view, originComponent)

        // Then
        verify { condition.handleEvent(rootView, view, listOf(onTrue), analyticsHandleEvent = AnalyticsHandleEvent(originComponent, "onTrue")) }
    }
    ...
    fun `execute should call onFalse attribute`() {
        ...
        every { condition.handleEvent(rootView, view, listOf(onFalse), analyticsHandleEvent = any()) } just Runs
