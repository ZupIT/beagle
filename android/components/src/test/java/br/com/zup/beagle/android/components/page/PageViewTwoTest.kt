...
class PageViewTwoTest : BaseComponentTest() {
    ...
    override fun setUp() {
        ...
        every { anyConstructed<ComponentsViewFactory>().makeViewPager(any()) } returns beaglePageView
        every { anyConstructed<ComponentsViewFactory>().makeBeagleFlexView(any(), capture(styleSlot)) } returns beagleFlexView
    ...
    fun buildView_should_make_and_addView_once_when_current_page_and_current_page_is_null() {
        ...
        // THEN
        verify(exactly = once()) { anyConstructed<ComponentsViewFactory>().makeViewPager(any()) }
        verify(atLeast = once()) { anyConstructed<ComponentsViewFactory>().makeBeagleFlexView(any(), styleSlot[0]) }
