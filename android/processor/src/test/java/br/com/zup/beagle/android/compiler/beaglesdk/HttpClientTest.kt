...
import br.com.zup.beagle.android.compiler.mocks.HTTP_CLIENT_IMPORT
...
import br.com.zup.beagle.android.compiler.mocks.VALID_HTTP_CLIENT_BEAGLE_SDK_FROM_REGISTRAR
import br.com.zup.beagle.android.compiler.mocks.VALID_THIRD_HTTP_CLIENT
...
internal class HttpClientTest : BeagleSdkBaseTest() {

    @TempDir
    lateinit var tempPath: Path

    @DisplayName("When register http client")
    @Nested
    inner class RegisterHttpClient {

        @Test
        @DisplayName("Then should add the http client in beagle sdk")
        fun testGenerateHttpClientCorrect() {
            // GIVEN
            val kotlinSource = SourceFile.kotlin(
                FILE_NAME, BEAGLE_CONFIG_IMPORTS + VALID_HTTP_CLIENT + SIMPLE_BEAGLE_CONFIG)
            ...
            val fileExpectedInString = VALID_HTTP_CLIENT_BEAGLE_SDK
    ...
    inner class RegisterFromOtherModule {
        @Test
        @DisplayName("Then should add the http client in beagle sdk")
        fun testGenerateHttpClientFromRegistrarCorrect() {
            // GIVEN
            every {
            ...
            val fileGeneratedInString = file.readText().replace(REGEX_REMOVE_SPACE, "")
            val fileExpectedInString = VALID_HTTP_CLIENT_BEAGLE_SDK_FROM_REGISTRAR
                .replace(REGEX_REMOVE_SPACE, "")
    ...
    @DisplayName("When register http client")
    @Nested
    inner class InvalidHttpClient {

        @Test
        @DisplayName("Then should show error with duplicate http client")
        fun testDuplicate() {
            // GIVEN
            val kotlinSource = SourceFile.kotlin(
                FILE_NAME, BEAGLE_CONFIG_IMPORTS + LIST_OF_HTTP_CLIENT + SIMPLE_BEAGLE_CONFIG)
            ...
            Assertions.assertTrue(compilationResult.messages.contains(MESSAGE_DUPLICATE_HTTP_CLIENT))
        }

        @Test
        @DisplayName("Then should show error with duplicate http client in PropertiesRegistrar")
        fun testDuplicateInRegistrar() {
            ...
            val kotlinSource = SourceFile.kotlin(FILE_NAME,
                BEAGLE_CONFIG_IMPORTS + VALID_HTTP_CLIENT + VALID_THIRD_HTTP_CLIENT + SIMPLE_BEAGLE_CONFIG
            )
            ...
            // THEN
            Assertions.assertTrue(compilationResult.messages.contains(MESSAGE_DUPLICATE_HTTP_CLIENT_REGISTRAR))
            assertEquals(KotlinCompilation.ExitCode.COMPILATION_ERROR, compilationResult.exitCode)
        }
    ...
    companion object {
        ...
        private const val MESSAGE_DUPLICATE_HTTP_CLIENT_REGISTRAR = "error: HttpClient defined multiple times: " +
            "1 - br.com.test.beagle.HttpClientTest " +
            "2 - br.com.test.beagle.HttpClientTestThree. " +
            "You must remove one implementation from the application."
    }
