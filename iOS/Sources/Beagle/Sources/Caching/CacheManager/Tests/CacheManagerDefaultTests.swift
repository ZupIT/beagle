...
final class CacheManagerDefaultTests: XCTestCase {
    ...
    func testSecondRecordDeletionFromDisk() {
        ...
        let diskCapacity = 2
        let diskManagerSpy = CacheDiskManagerSpy()
        let dependencies = CacheManagerDependencies(logger: BeagleLoggerDumb(), cacheDiskManager: diskManagerSpy)
        let manager = CacheManagerDefault(dependencies: dependencies, config: .init(memoryMaximumCapacity: memoryCapacity, diskMaximumCapacity: diskCapacity, cacheMaxAge: 10))
        let cacheReference1 = CacheReference(identifier: url1, data: jsonData, hash: defaultHash)
        let cacheReference2 = CacheReference(identifier: url2, data: jsonData, hash: defaultHash)
        let cacheReference3 = CacheReference(identifier: url3, data: jsonData, hash: defaultHash)
        ...
        //Then
        XCTAssertTrue(diskManagerSpy.callRemoveLastUsed)
...
class CacheDiskManagerSpy: CacheDiskManagerProtocol {
    var references = [String: CacheReference]()
    var callRemoveLastUsed = false
    var callSaveChanges = false
    
    func removeLastUsed() {
        callRemoveLastUsed = true
    }
    func saveChanges() {
    ...
    func clear() {
        references = [String: CacheReference]()
    }
}
