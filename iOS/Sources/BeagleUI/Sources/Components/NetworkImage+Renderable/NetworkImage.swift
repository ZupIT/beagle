...
extension NetworkImage: Widget {
    ...
    public func toView(renderer: BeagleRenderer) -> UIView {
        ...
        if let placeholder = placeholder {
            let imagePlaceholder = placeholder.toView(renderer: renderer)
            imagePlaceholder.beagle.setup(self)
            renderer.context.lazyLoadManager.lazyLoadImage(path: path, placeholderView: imagePlaceholder, imageView: imageView, flex: flex ?? Flex())
