...
Feature: SetContext Action Validation
    ...
    Scenario Outline: SetContext 01 - A context value should change when a SetContext action is triggered setting a hardcoded value on that context
    ...
    Scenario Outline: SetContext 02 - A context value should change when a SetContext action is triggered setting value via expression on that context
        When I press a SetContext button with the "<buttonTitle>" title
        Then a text with the "<message>" message should appear on the screen
