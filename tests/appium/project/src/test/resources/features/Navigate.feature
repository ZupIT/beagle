...
# FALHANDO: PopView nao fecha a modal
...
# FALHANDO: nao acontece nada ao clicar nos titulos abaixo
#    Scenario Outline: Navigation 09 - 'PushStack, ResetStack, ResetApplication' This component could be called
#    from an other Beagle Server Driven Activity (SDA) rather than the basic one. The new Beagle Server Driven Activity
#    must be annotated with the "@RegisterController" annotation that will hold de controllerId that must be listed on
#    the action above. The retry button is called only on this other Beagle Activity
#
#        When I press a navigation failure button <title>
...
# FALHANDO: nao acontece nada ao clicar nos titulos abaixo
#    Scenario Outline: Navigation 10 - 'ResetStack, ResetApplication' These components could be called from a different
#    Beagle Activity rather than the basic one. Here they are called from the Default Beagle Server Driven Activity. When
#    this happens they must trigger a TRY AGAIN button on a failed attempt.
#
#        When I press a navigation failure button <title>
